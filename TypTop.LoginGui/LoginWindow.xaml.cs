...
namespace TypTop.Gui
{
    ...
    public partial class LoginWindow : Window
    {
        ...
        {
            using var unitOfWork = new UnitOfWork(new ContextFactory().CreateDbContext(null));

            if (PasswordBox.Password != "" && unitOfWork.Users.GetWhere(u => u.Username.Equals(UsernameBox.Text)).Any())
            {
                var salt = unitOfWork.Users.GetSalt(UsernameBox.Text);
                var hash = unitOfWork.Users.GetPasswordHash(UsernameBox.Text);
        ...
        /// <summary>
        ///     Changes view to window in which the user enters the username and password of a new account and adds it to the
        ///     database if valid.
        /// </summary>
                            ...
                            if (!unitOfWork.Users.GetWhere(u => u.Username.Equals(CreationUsernameBox.Text)).Any())
                            {
                                ...
                                unitOfWork.Users.Add(new User
                                {
                                    ...
                                    Password = Convert.ToBase64String(
                                        PasswordHasher.HashPassword(CreationPasswordBox.Password, salt)),

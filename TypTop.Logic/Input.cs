...
namespace TypTop.Logic
{
    public abstract class Input
    {
        ...
        /// <summary>
        ///     Ignores numbers when true, does not count them as mistake when entered. Removes all numbers from words if they
        ///     contain any.
        /// </summary>
        ...
        /// <summary>
        ///     Ignores punctuation when true, does not count them as mistake when entered. Removes all interpunction from words if
        ///     they contain any.
        /// </summary>
        ...
        /// <summary>
        ///     Ignores space when true, does not count as mistake when entered. Removes all whitespaces from words if they contain
        ///     any.
        /// </summary>
        ...
        /// <summary>
        ///     Remove the current word when the spacebar is pressed. Only works when IgnoreSpace is false. Note that when a list
        ///     is used, all words will be removed when none of the words match.
        /// </summary>
        ...
        public void TextInput(string letters)
        {
            if (letters.Equals("\b"))
            {
                if (AllowBackspace) Backspace();
            }
            else
            {
                foreach (var ch in letters) TextInput(ch);
            }
        }
        ...
        /// </param>
        /// <param name="input">
        ///     If set, checks a specific letterindex of the word. If not, it takes the current index and when necessary adjusts
        ///     it.
        /// </param>
        /// <returns>
        ...
        {
            if (CheckIgnoredChars(letter)) return true;

            if (word == null) return false;

            var index = input ?? word.Index;
            var wordCharAtIndex = word.Letters[index];
            ...
            {
                if (input == null) word.Correct = true;

                word.Finished = true;
                return true;
            }

            var result = letter == wordCharAtIndex;
            ...
            if (char.IsLetter(letter))
            {
                ...
                result = CaseSensitive && letter == wordCharAtIndex ||
                         !CaseSensitive && char.ToLower(letter) == char.ToLower(wordCharAtIndex);
            }

            if (input == null) word.Index = index + 1;

            if (result && index == word.Letters.Length - 1) word.Finished = true;
        ...
        {
            return IgnoreNumbers && char.IsDigit(ch) || IgnoreSpace && char.IsWhiteSpace(ch) ||
                   IgnorePunctuation && char.IsPunctuation(ch);
        }
        ...
        {
            var from = "àèìòùÀÈÌÒÙ äëïöüÄËÏÖÜ âêîôûÂÊÎÔÛ áéíóúÁÉÍÓÚðÐýÝ ãñõÃÑÕšŠžŽçÇåÅøØ".ToCharArray();
            var to = "aeiouAEIOU aeiouAEIOU aeiouAEIOU aeiouAEIOUdDyY anoANOsSzZcCaAoO".ToCharArray();
